...
widget::~widget() {
    if (model) free_result_struct(model);
}
...
void widget::paintGL() {
    ...
    if (model) {
        ...
        glVertexPointer(3, GL_DOUBLE, 0, model->pointsCoordinates);
        glDrawElements(GL_LINES, model->numConnections, GL_UNSIGNED_INT, model->connectionIndices);
        ...
        if (dot_type) {
            ...
            glDrawArrays(GL_POINTS, 0, model->numPoints / 3);
